...
- 选择任意本地 Git 工作区；
- 自动记录最近使用的仓库并在下次启动时恢复，失效记录可从欢迎页移除；
- 读取本地分支、远程跟踪分支及提交作者；
- 指定源分支和目标分支；
- 按作者、日期和文件路径筛选；
- 严格 SHA、补丁等价两种检查模式；
...
应用会在本机设置文件中保存最近打开的 8 个仓库路径，并把最后一次使用的仓库设为启动默认值。下次打开应用时会自动恢复该仓库；如果目录被移动、删除或不再是 Git 工作区，应用会返回欢迎页并显示最近记录，支持重新打开或移除失效条目。仓库记录与 API Key 分开处理，不会上传到远程服务。
